...
Adds a Contact Us footer button and popup form to every root HTML page.
Installs a separate Cloudflare contact Worker.
This PowerShell source is ASCII-only for Windows PowerShell 5.1 compatibility.
#>
...
function Write-Utf8FileAtomic {
    ...
    $TemporaryPath = "$Path.contact-v1.7.0.tmp"
...
function Remove-GeneratedBlock {
    ...
    while ($true) {
        ...
        if ($EndIndex -lt 0) {
            throw "Generated block start marker exists without an end marker: $StartMarker"
        }
...
if ($HtmlFiles.Count -eq 0) {
    throw "No root HTML pages were found in: $ResolvedRepoRoot"
}
...
Write-Host ""
Write-Host "Backing up current Crack Packs files..." -ForegroundColor Cyan

foreach ($HtmlFile in $HtmlFiles) {
    $BackupPath = Join-Path $BackupRoot $HtmlFile.Name
...
Write-Host ""
Write-Host "Installing complete contact system files..." -ForegroundColor Cyan
...
foreach ($HtmlFile in $HtmlFiles) {
    ...
    $Updated = Remove-GeneratedBlock `
        -Text $Updated `
        -StartMarker $AssetStart `
        -EndMarker $AssetEnd
    ...
    $AssetBlock = (
        "  $AssetStart" + $NewLine +
        "  <link rel=""stylesheet"" href=""assets/css/contact-form.css?v=1.7.0"">" + $NewLine +
        "  $AssetEnd" + $NewLine
    )
    ...
    $ModalBlock = (
        ...
        "        </label>" + $NewLine +
        "        <label class=""contact-honeypot"" aria-hidden=""true"">" + $NewLine +
    ...
    $ScriptBlock = (
        "$ScriptStart" + $NewLine +
        "<script src=""assets/js/contact-form.js?v=1.7.0""></script>" + $NewLine +
        "$ScriptEnd" + $NewLine
    )

    $Updated = Insert-BeforeTag `
        -Text $Updated `
        -ClosingTag "</head>" `
        -Block $AssetBlock
    ...
    $Checks = @(
        ...
        "assets/js/contact-form.js?v=1.7.0"
    )

    foreach ($Check in $Checks) {
        if (-not $Updated.Contains($Check)) {
            throw "HTML verification failed for $($HtmlFile.FullName): missing $Check"
        }
    }
...
Write-Host ""
Write-Host "Contact form installation v1.7.0 complete." -ForegroundColor Green
    ...
    if ($PendingChanges.Count -gt 0) {
        Invoke-Checked `
            ...
            -Arguments @(
                "commit",
                "-m",
                "Add secure Contact Us form and email Worker"
            )
        ...
        Write-Host "No Git changes were staged; commit and push skipped." -ForegroundColor Yellow
    }
}

Write-Host ""
Write-Host "Validation URLs:" -ForegroundColor Cyan
Write-Host "  https://contact-api.crackpacks.com/health"
Write-Host "  https://crackpacks.com"
