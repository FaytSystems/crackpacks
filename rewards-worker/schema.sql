...
CREATE TABLE IF NOT EXISTS seller_store_listings (
  ...
  product_category_key TEXT NOT NULL DEFAULT 'tcg' CHECK(length(product_category_key) <= 64),
  shipping_weight_profile_id TEXT,
  fixed_shipping_cents INTEGER NOT NULL DEFAULT 0 CHECK(fixed_shipping_cents BETWEEN 0 AND 10000000),
  shipping_overage_policy TEXT NOT NULL DEFAULT 'seller_pays_difference' CHECK(shipping_overage_policy IN ('seller_pays_difference')),
...
CREATE INDEX IF NOT EXISTS idx_employee_invitations_target ON employee_invitations(target_email,expires_at);
...
CREATE INDEX IF NOT EXISTS idx_employee_profiles_status ON employee_profiles(status,updated_at);
...
CREATE INDEX IF NOT EXISTS idx_employee_time_entries_member ON employee_time_entries(member_id,work_date DESC,created_at DESC);
CREATE INDEX IF NOT EXISTS idx_employee_time_entries_status ON employee_time_entries(status,work_date DESC);
...
CREATE TABLE IF NOT EXISTS internal_email_messages (
  ...
  category TEXT NOT NULL DEFAULT 'direct' CHECK(category IN ('direct','employment')),
...
CREATE INDEX IF NOT EXISTS idx_internal_email_messages_sender ON internal_email_messages(sender_member_id,created_at DESC);
CREATE INDEX IF NOT EXISTS idx_internal_email_messages_recipient ON internal_email_messages(recipient_member_id,created_at DESC);
