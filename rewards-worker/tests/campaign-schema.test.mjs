...
const employeeAccountsMigration = readFileSync(new URL("../migrations/0044_add_employee_accounts_and_internal_email.sql", import.meta.url), "utf8");
...
test("campaign schema and sequential campaign migrations are valid", () => {
  ...
  migrationDb.exec(employeeAccountsMigration);
  ...
  assert.ok(migrationDb.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='employee_invitations'`).get());
  assert.ok(migrationDb.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='employee_profiles'`).get());
  assert.ok(migrationDb.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='employee_time_entries'`).get());
  assert.ok(migrationDb.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='internal_email_messages'`).get());
  migrationDb.close();
});
